...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val btnSubirPDF = findViewById<Button>(R.id.btnSubirPDF)
        ...
        btnSubirPDF.setOnClickListener {
            seleccionarPDF()
        }
        ...
        btnGuardar.setOnClickListener {
            ...
            if (correo.isNotEmpty() && clave.isNotEmpty() && imageUri != null) {
                guardarUsuarioConFoto(correo, clave, imageUri, pdfUri.toString())
            } else {
                Toast.makeText(
                    this,
    ...
    private fun guardarUsuarioConFoto(correo: String, clave: String, imageUri: String, pdfUri: String) {
        try {
            GlobalScope.launch(Dispatchers.IO) {
                val objConexion = ClaseConexion().cadenaConexion()
                val statement =
                    objConexion?.prepareStatement("INSERT INTO tbMisUsuarios (UUID, correo, contrasena, FotoURI, PDFURI) VALUES (?, ?, ?, ?, ?)")!!
                statement.setString(1, uuid)
                statement.setString(2, correo)
                statement.setString(3, clave)
                statement.setString(4, imageUri)
                statement.setString(5, pdfUri)  // Guardamos la URL del PDF
